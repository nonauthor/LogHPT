...
def get_labels(predictions, references, tokens, device, id_to_label, tokenizer,
               ):
...
def evaluate(metric, model, tokenizer, eval_dataloader, device,pad_to_max_length,
             id_to_label, ):
    model.eval()
    start = time.time()
    token_list = []
...
def evaluate_dual(metric, model_key,model_dual, tokenizer, eval_dataloader, device,pad_to_max_length,
             id_to_label, ):
    model_key.eval()
    model_dual.eval()
    start = time.time()
