...
class multi_task_dataset_zmj(Dataset):
    def __init__(self,root_path,dataname_list:list,train=True):
        super().__init__()
        self.logs = []
        self.templates = []
        ...
        for dataname in dataname_list:
            if train==True:
                path = root_path+f'\\{dataname}_train_{200}.json'
            else:
                path = root_path+f'\\{dataname}_test_{200}.json'
            data = read_json(path)
            self.logs.extend(data[0])
            self.templates.extend(data[1])
        self.datas = list(zip(self.logs,self.templates))
